...
from brenda import aws, gcp, utils, error
...
def start_output_push_process(opts, args, conf, outdir):
    p = Multiprocess(target=file_push_process, args=(opts, args, conf, outdir))
    p.start()
    return p


def file_push_process(opts, args, conf, outdir):
    def do_file_push():
        use_gcp = None
        if conf.get('GCP_STORAGE', '0') is '0':
            use_gcp = False
        else:
            use_gcp = True
        if use_gcp:
        ...
        for dirpath, dirnames, filenames in os.walk(outdir):
            for f in filenames:
                ...
                if use_gcp:
                    print("PUSH", path, "TO", gcp.format_gs_url(bucktup, f))
                    gcp.put_gs_file(conf, bucktup, path, f)
                else:
    ...
    try:
        error.retry(conf, do_file_push)
    except Exception as e:
        print("File push failed:", e)
        raise e
    sys.exit(0)
...
def run_tasks(opts, args, conf):
        ...
        try:
            ...
            while True:
                ...
                if local.task_active:
                    local.task_active.proc = start_output_push_process(opts, args, conf, local.task_active.outdir)
    ...
    #GCP thing
    if conf.get('GCP_STORAGE', '0') is '0':
        local.use_gcpstore = False
    else:
        local.use_gcpstore = True
    ...
    if local.use_gcpstore:
        gcp.get_gs_output_bucket(conf)
    else:
    ...
    # get project (from s3:// or file://), or gs://
...
def get_blend_project(conf, blendurl, proj_dir):
    # target file in which to save S3 download
    fn = os.path.basename(blendurl)
    ...
    try:
        with utils.Cd(new_dir) as cd:
            ...
            if blendurl.startswith("gs://"):
                file_len, etag = gcp.gs_get(conf, blendurl, fn, etag=etag)
            else:
                file_len, etag = aws.s3_get(conf, blendurl, fn, etag=etag)
...
def get_project(conf, url):
    if url.startswith("file://"):
        ...
        if ebs_snap:
            ...
            get_blend_project(conf, url, proj_dir)
